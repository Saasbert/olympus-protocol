...
export function OlympusHub() {
  ...
  const isInBattle = location.pathname === '/olympus/battle'
  ...
  return (
    <div className="flex flex-col h-full">
      <nav className="flex gap-2 px-4 py-2 bg-black/20">
        {subTabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors min-h-touch ${
                active ? 'bg-mecha-cyan text-black' : 'text-white/50 hover:text-white'
              }`}
            >
              {tab.label}
            </button>
